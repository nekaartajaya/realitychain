...
mod tests {
    ...
    fn setup_contract() -> (VMContextBuilder, RealityParcelsContract) {
        ...
        let contract = RealityParcelsContract::new_default_meta(
            accounts(0),
            accounts(1),
            accounts(2),
            accounts(3),
    ...
    fn test_invalid_buy_price_null() {
        ...
        contract.nft_set_series_stake_amount(
            "1".to_string(),
            near_sdk::json_types::U128::from(57142857140),
            8,
        );
    ...
    fn test_nft_transfer_payout() {
        ...
        payout_calc.insert(
            accounts(1),
            U128::from((2000 * (1 * 10u128.pow(24))) / 10_000),
        );
        payout_calc.insert(
            accounts(2),
            U128::from((8000 * (1 * 10u128.pow(24))) / 10_000),
        );
